...
// Generate the small trig values for fft_HEIGHT plus optionally trig values used in pairSq.
vector<float2> genSmallTrigComboFP32(u32 width, u32 middle, u32 size, u32 radix, bool tail_single_wide, u32 tail_trigs) {
...
// Generate the small trig values for fft_HEIGHT plus optionally trig values used in pairSq.
vector<ulong2> genSmallTrigComboGF61(u32 width, u32 middle, u32 size, u32 radix, bool tail_single_wide, u32 tail_trigs) {
...
#if FFT_FP32
  vector<float2> tab1 = genSmallTrigComboFP32(width, middle, size, radix, tail_single_wide, tail_trigs);
...
#if NTT_GF61
  vector<ulong2> tab3 = genSmallTrigComboGF61(width, middle, size, radix, tail_single_wide, tail_trigs);
...
TrigPtr TrigBufCache::smallTrig(u32 width, u32 nW, u32 middle, u32 height, u32 nH, u32 variant, bool tail_single_wide, u32 tail_trigs) {
  lock_guard lock{mut};
  auto& m = small;
  TrigPtr p{};

  // See if there is an existing smallTrigCombo that we can return (using only as subset of the data)
  ...
  if (width == height && nW == nH) {
    decay_t<decltype(m)>::key_type key{height, nH, width, middle, tail_single_wide, tail_trigs};
    auto it = m.find(key);
    if (it != m.end() && (p = it->second.lock())) return p;
  }

  // See if there is an existing non-combo smallTrig that we can return
  decay_t<decltype(m)>::key_type key{width, nW, 0, 0, 0, 0}; 
  auto it = m.find(key);
  if (it != m.end() && (p = it->second.lock())) return p;

  // Create a new non-combo
  p = make_shared<TrigBuf>(context, genSmallTrig(width, nW));
  m[key] = p;
  smallCache.add(p);
  return p;
}

TrigPtr TrigBufCache::smallTrigCombo(u32 width, u32 middle, u32 height, u32 nH, u32 variant, bool tail_single_wide, u32 tail_trigs) {
  if (tail_trigs == 2 && !NTT_GF31 && !NTT_GF61)             // No pre-computed trig values.  We might be able to share this trig table with fft_WIDTH
    return smallTrig(height, nH, middle, height, nH, variant, tail_single_wide, tail_trigs);
  ...
  if (it == m.end() || !(p = it->second.lock())) {
    p = make_shared<TrigBuf>(context, genSmallTrigCombo(width, middle, height, nH, tail_single_wide, tail_trigs));
    m[key] = p;
    smallCache.add(p);
  }
  return p;
